<script>
	import EntityExplorer from '@app/ui/EntityExplorer.svelte';
	import InboxList from '@app/inbox/InboxList.svelte';
	import NotesList from '@app/notes/NotesList.svelte';
	import ActivitiesList from '@app/activity/ActivitiesList.svelte';
	import PlaceholderInfo from '@app/ui/PlaceholderInfo.svelte';
	import WorldName from '@app/world/WorldName.svelte';

	export let world;

	$: inboxNotes = $world.spaces[0].notes; // TODO temporarily hardcoded
	$: notesNotes = $world.spaces[0].notes; // TODO temporarily hardcoded
	$: activities = $world.spaces[2].activities; // TODO temporarily hardcoded
</script>

<PlaceholderInfo>
	this is one of your avatars,
	<WorldName name={$world.slug} isMention={true} />
</PlaceholderInfo>
<PlaceholderInfo>
	avatars are like characters in a story, pseudonyms that you control
</PlaceholderInfo>
<PlaceholderInfo>
	on the left, you'll see your avatar's spaces - click around to learn more
</PlaceholderInfo>
<PlaceholderInfo>
	TODO show all activity for this avatar & interact with it
</PlaceholderInfo>

<!-- TODO this is temp  -->
<InboxList notes={inboxNotes} />
<NotesList notes={notesNotes} />
<ActivitiesList {activities} />

{#if $world.content}{$world.content}{/if}
