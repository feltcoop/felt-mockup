<script>
	import Content from '@app/ui/Content.svelte';
	import Chat from '@app/chat/Chat.svelte';
	import Forum from '@app/forum/Forum.svelte';
	import Blog from '@app/blog/Blog.svelte';
	import Inbox from '@app/inbox/Inbox.svelte';
	import Notes from '@app/notes/Notes.svelte';
	import Activities from '@app/activity/Activities.svelte';
	import ErrorMessage from '@app/ui/ErrorMessage.svelte';

	export let world;
	export let space;
</script>

<!-- TODO where should this padding ideally go? -->
<div class="flex flex-col flex-1 p-2" style="max-width: 640px;">
	<!-- TODO componentize - SpaceBreadcrumb or PathBreadcrumb or SlugBreadcrumb -->
	<Content>
		<h1 class="flex">
			<a
				class="bg-green-100 px-4 rounded-bl-lg border-dashed border-l-4
				border-t-2 border-b-2 border-green-200"
				href={$world.slug}>
				{$world.title}
			</a>
			<!--
				TODO this `/` should be a thick chunky line that is
				centered at the junction between the two path items,
				and dark green on the left half and dark purple on the right.
				(how? blending? svg? canvas? animate it into place?)
			-->
			<div
				class="w-0 z-10 text-gray-500 relative"
				style="left: -5px; font-weight: bold;">
				/
			</div>
			<a
				class="active-space-link bg-purple-100 px-4 border-r-4 border-t-2
				border-b-2 border-purple-200 cursor-default rounded-tr-lg border-dashed "
				href="{$world.slug}/{$space.slug}">
				{$space.title}
			</a>
		</h1>
		{#if $space.description}
			{@html $space.description}
		{/if}
	</Content>

	<div class="flex-1">
		{#if $space.type === 'chat'}
			<Chat messages={$space.messages} />
		{:else if $space.type === 'forum'}
			<Forum topics={$space.topics} />
		{:else if $space.type === 'blog'}
			<Blog posts={$space.posts} />
		{:else if $space.type === 'inbox'}
			<Inbox notes={$space.notes} />
		{:else if $space.type === 'notes'}
			<Notes notes={$space.notes} />
		{:else if $space.type === 'activities'}
			<Activities activities={$space.activities} />
		{:else}
			<ErrorMessage message={`Unknown space type "${$space.type}"`} />
		{/if}
	</div>
</div>

<style lang="postcss">
	.active-space-link {
		@apply text-purple-400 !important;
	}
	a {
		position: relative;
	}
	a:active {
		top: 1px;
		@apply border-green-300;
	}
	.active-space-link:active {
		top: -1px;
		@apply border-purple-300;
	}
</style>
